...
# -*- coding: utf-8 -*-
"""
Raspberry Pi 5 Environmental Data Collection Module
Version: 1.0.0

This module receives environmental data from P2 and P3 Pico devices via WiFi,
...
Usage:
    python3 data_collector.py [--port PORT] [--data-dir DIR]
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/var/log/data_collector.log"),
...
class DataCollector:
        ...
        for device in ["P2", "P3"]:
            ...
            if not file_exists:
                self.csv_writers[device].writerow([
                    ...
                    "pressure", "gas_resistance", "co2_level"
                ])
    ...
    def _rotate_csv_files(self):
        ...
        for device, file in self.csv_files.items():
            # Close current file
            file.close()
    ...
    def _validate_data(self, data):
        ...
        if not (0 <= float(data["co2_level"]) <= 10000):
            logger.warning(f"CO2 level out of range: {data['co2_level']}")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Raspberry Pi 5 Environmental Data Collector")
