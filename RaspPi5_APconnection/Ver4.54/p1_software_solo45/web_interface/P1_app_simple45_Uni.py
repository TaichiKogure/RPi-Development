...
def jsonify_numpy(obj):
    """Convert NumPy arrays and datetime objects to Python lists and strings for JSON serialization.

    Args:
        obj: The object to convert (can be a dict, list, NumPy array, or datetime object)

    Returns:
        The converted object with NumPy arrays replaced by Python lists and datetime objects replaced by strings
    """
...
def get_connection_status():
    ...
    try:
        ...
        response = requests.get(f"{DEFAULT_CONFIG['connection_monitor_api_url']}/api/connection/latest", timeout=5)
...
def get_latest_data():
    ...
    try:
        ...
        response = requests.get(f"{DEFAULT_CONFIG['data_collector_api_url']}/api/data/latest", timeout=5)
        if response.status_code == 200:
            return jsonify(response.json())
    except Exception as e:
...
def read_csv_data(csv_path, days=1):
    ...
    try:
        ...
        # Keep timestamp as datetime object for Plotly
        # Plotly expects datetime objects for the x-axis
        # The conversion to string will happen in the jsonify_numpy function if needed
...
def get_latest_logs():
    ...
    try:
        log_file = "/var/log/p1_startup_solo45_Uni.log"
        ...
        return jsonify({"error": f"ログファイル読み込みエラー: {e}"}), 500
