...
class BME680Sensor:
    ...
    def _set_gas_heater(self, temperature=None, duration=None, enable=False):
        """Set the gas heater temperature and duration."""
        if enable:
            if temperature is None:
                temperature = self.gas_settings.get('heatr_temp', 320)
            if duration is None:
                duration = self.gas_settings.get('heatr_dur', 150)
        
        # デバッグ表示
        print(f"Debug: Setting heater -> Temperature: {temperature} Duration: {duration}")
        print(f"Debug (Pre-check): Temperature={temperature}, Duration={duration}")

        # Temperature and duration must be within 0-255
        temperature = min(max(0, temperature), 255)
        duration = min(max(0, duration), 255)

        print(f"Debug (Post-check): Temperature={min(max(0, temperature), 255)}, Duration={min(max(0, duration), 255)}")

        # 書き込み
        self._write_byte(0x5A, temperature)  # Temperature
        self._write_byte(0x64, duration)    # Duration
    ...
    def _calc_temperature(self, temp_adc):
        ...
        var2 = ((temp_adc / 131072.0 - self.calibration_data['par_t1'] / 8192.0) ** 2) * self.calibration_data[
            'par_t3'] * 16
    ...
    def _calc_pressure(self, pres_adc):
        ...
        var1 = (self.calibration_data['par_p3'] * var1 * var1 / 16384.0 + self.calibration_data[
    ...
    def _calc_humidity(self, hum_adc):
        ...
        var2 = var1 * (self.calibration_data['par_h2'] / 262144.0 * (
                    1.0 + self.calibration_data['par_h4'] / 16384.0 * temp_comp + self.calibration_data[
    ...
    def get_readings(self):
        ...
        if self.power_mode == 1:
            # センサを forced mode に設定しガス測定を有効にする
            reg = self._read_byte(BME680_CTRL_MEAS_ADDR)
            self._write_byte(BME680_CTRL_MEAS_ADDR, reg & 0xFC | 0x01)
        ...
        # デバッグ用: `gas_valid` と `heat_stable` の値を表示
        print(f"Debug: gas_valid={raw_data['gas_valid']}, heat_stable={raw_data['heat_stable']}")
    ...
    def measure(self, gas=False):
        """計測指示（gas=Trueでガス加熱付き測定）"""
        if gas:
            # ガスヒーターを明示的にオンにする
            self._set_gas_heater(enable=True)
        else:
            # ガスヒーターを明示的にオフにする
            self._set_gas_heater(enable=False)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # --- 変更後 ---
            # まず、ガス計測用のヒーター加熱を明示的に有効化
            sensor.measure(gas=True)  # ガス測定のため加熱＆計測要求
            readings = sensor.get_readings()  # 通常通り計測値取得
            ...
            if 'gas' in readings:
                print(f"Gas: {readings['gas']:.2f} Ohms")
            else:
                print("Gas measurement not available")
