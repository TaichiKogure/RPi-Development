...
# -*- coding: utf-8 -*-
"""
Raspberry Pi 5 Access Point Setup Script for Solo Version 4.5
Version: 4.5.0-solo
...
Usage:
    sudo python3 P1_ap_setup_solo45.py [--configure | --enable | --disable | --status]
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/var/log/ap_setup_solo45.log"),
...
CONFIG_PATH = Path("/etc/raspap_solo45")
HOSTAPD_CONF = "/etc/hostapd/hostapd.conf"
DNSMASQ_CONF = "/etc/dnsmasq.conf"
DHCPCD_CONF = "/etc/dhcpcd.conf"
SYSCTL_CONF = "/etc/sysctl.conf"
INTERFACES_CONF = "/etc/network/interfaces"
RASPAP_CONFIG = str(CONFIG_PATH / "raspap_solo45.conf")
...
def configure_hostapd(config):
    ...
    hostapd_config = f"""# hostapd configuration file for Ver4.5
...
def configure_dnsmasq(config):
    ...
    dnsmasq_config = f"""# dnsmasq configuration for Raspberry Pi 5 AP Solo Ver4.5
interface={config['ap_interface']}
    ...
    try:
        ...
        if "net.ipv4.ip_forward=1" not in sysctl_content:
            with open(SYSCTL_CONF, 'a') as f:
                f.write("\n# Enable IP forwarding for AP Ver4.5\nnet.ipv4.ip_forward=1\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Raspberry Pi 5 Access Point Setup for Solo Version 4.5")
