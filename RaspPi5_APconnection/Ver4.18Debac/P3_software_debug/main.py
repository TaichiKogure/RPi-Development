...
# -*- coding: utf-8 -*-
"""
Raspberry Pi Pico 2W Main Program for Environmental Monitoring - Debug Version 4.18
...
def initialize():
    ...
    try:
        ...
        # Print header
        print(f"=== Raspberry Pi Pico 2W Environmental Monitor Ver4.18 ({DEVICE_ID}) ===")
        print("Initializing...")
...
def main_loop(bme, co2_sensor, client, transmitter, watchdog):
    ...
    if client:
        ...
        if not connected and not DEBUG_DISABLE_AUTO_RESET:
    ...
    try:
        while True:
            try:
                ...
                # Transmit data if transmitter is available
                if transmitter:
                    transmitter.collect_and_send_data()
                
                # Print sensor readings if available
                if bme and time.time() - last_feed_time > 10:  # Print every 10 seconds
