...
# -*- coding: utf-8 -*-
"""
Raspberry Pi 5 Access Point Setup Script for Solo Version 4.44
Version: 4.44.0-solo
...
Usage:
    sudo python3 P1_ap_setup_solo44.py [--configure | --enable | --disable | --status]
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/var/log/ap_setup_solo44.log"),
...
CONFIG_PATH = Path("/etc/raspap_solo44")
HOSTAPD_CONF = "/etc/hostapd/hostapd.conf"
DNSMASQ_CONF = "/etc/dnsmasq.conf"
DHCPCD_CONF = "/etc/dhcpcd.conf"
SYSCTL_CONF = "/etc/sysctl.conf"
INTERFACES_CONF = "/etc/network/interfaces"
RASPAP_CONFIG = str(CONFIG_PATH / "raspap_solo44.conf")
...
def configure_hostapd(config):
    ...
    hostapd_config = f"""# hostapd configuration file for Ver4.44
...
def configure_dnsmasq(config):
    ...
    dnsmasq_config = f"""# dnsmasq configuration for Raspberry Pi 5 AP Solo Ver4.44
interface={config['ap_interface']}
    ...
    try:
        ...
        if "net.ipv4.ip_forward=1" not in sysctl_content:
            with open(SYSCTL_CONF, 'a') as f:
                f.write("\n# Enable IP forwarding for AP Ver4.44\nnet.ipv4.ip_forward=1\n")
...
def check_status():
    ...
    # Print status
    print("\nアクセスポイントの状態:")
    print(f"hostapd サービス: {status['hostapd']}")
    print(f"dnsmasq サービス: {status['dnsmasq']}")
    print(f"IP設定: {status['ip_configured']}")
    ...
    if status["hostapd"] == "active" and status["dnsmasq"] == "active" and status["ip_configured"] == "Yes":
        ...
        config = load_config()
        print(f"\nSSID: {config['ap_ssid']}")
        print(f"パスワード: {config['ap_password']}")
        print(f"IPアドレス: {config['ap_ip']}")
    else:
        print("\nアクセスポイントが正常に動作していません。")
...
def configure_ap():
    ...
    print(f"パスワード: {config['ap_password']}")
    print(f"IPアドレス: {config['ap_ip']}")
    print("\n変更を有効にするためにRaspberry Piを再起動してください。")

    return True

def main():
    """Main function to parse arguments and execute commands."""
    parser = argparse.ArgumentParser(description="Raspberry Pi 5 Access Point Setup for Solo Version 4.44")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--configure", action="store_true", help="アクセスポイントを設定する")
    group.add_argument("--enable", action="store_true", help="アクセスポイントを有効にする")
    group.add_argument("--disable", action="store_true", help="アクセスポイントを無効にする")
    group.add_argument("--status", action="store_true", help="アクセスポイントの状態を確認する")
