...
def initialize():
    ...
    try:
        ...
        debug_print("Skipping MH-Z19C CO2 sensor initialization...")
        # from mhz19c import MHZ19C
        # co2_uart = UART(1, tx=Pin(UART_TX_PIN), rx=Pin(UART_RX_PIN), baudrate=9600)
        # co2_sensor = MHZ19C(co2_uart)
        co2_sensor = None  # Set to None to avoid errors
        debug_print("CO2 sensor disabled for BME680-only operation")
        ...
        debug_print("Data transmitter initialized for BME680 only")
    ...
    while True:
        try:
            ...
            if current_time - last_transmission_time >= TRANSMISSION_INTERVAL:
                ...
                # Ver2.00zeroOne: Commented out CO2 readings as we're disabling CO2 sensor functionality
                # CO2 readings
                # co2 = co2_sensor.read_co2()
                # debug_print(f"CO2: {co2} ppm")
                co2 = None  # Set to None as CO2 sensor is disabled
                
                # Prepare data for transmission
                # Ver2.00zeroOne: Removed CO2 data from transmission dictionary as we're disabling CO2 sensor functionality
                data = {
                    "device_id": DEVICE_ID,
