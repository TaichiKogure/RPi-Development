...
FALLBACK_DEFAULT_CONFIG = {
    ...
    # Add P2/P3 directories (in addition to existing P4+)
...
FALLBACK_MONITOR_CONFIG = {
    "devices": {
        "P2": {"ip": None, "mac": None, "channel": 6},
        "P3": {"ip": None, "mac": None, "channel": 6},
        "P4": {"ip": None, "mac": None, "channel": 6},
        "P5": {"ip": None, "mac": None, "channel": 6},
        "P6": {"ip": None, "mac": None, "channel": 6}
    },
    "update_interval": 5,
    "ping_count": 3,
    "ping_timeout": 1
...
class DataCollector:
    ...
    def __init__(self, config=None):
        ...
        # Ensure P2/P3 directories as well as existing ones
        for key in ["rawdata_p2_dir", "rawdata_p3_dir", "rawdata_p4_dir", "rawdata_p5_dir", "rawdata_p6_dir"]:
            if key in self.config and self.config[key]:
                os.makedirs(os.path.join(self.config["data_dir"], self.config[key]), exist_ok=True)
    ...
    def _init_csv_files(self):
        ...
        # Define supported devices (include P2/P3)
    ...
    def _rotate_csv_files(self):
        ...
        for device in ["P2", "P3", "P4", "P5", "P6"]:
            # Close current date-based file (if initialized)
    ...
    def _store_data(self, data):
        ...
        # Log concrete file paths for early detection
        try:
            date_csv_path = self.csv_files[device_id].name
            fixed_csv_path = self.fixed_csv_files[device_id].name
            logger.info(f"Saved {device_id} -> date_csv: {date_csv_path} | fixed_csv: {fixed_csv_path}")
        except Exception:
            pass
