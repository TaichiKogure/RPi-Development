...
All information is displayed on a single screen in a compact format.
...
ROW_HEIGHT = 9   # Height between rows (slightly reduced for better fit)
COL_WIDTH = 64   # Width for each column
X_COL1 = 0       # X position for column 1 (Date/Time)
X_COL2 = 0       # X position for column 2 (Temperature)
X_COL3 = 64      # X position for column 3 (Humidity)
X_COL4 = 0       # X position for column 4 (Abs Humidity)
X_COL5 = 64      # X position for column 5 (Pressure)
X_COL6 = 0       # X position for column 6 (Gas)
    ...
    try:
        ...
        while True:
            try:
                ...
                # Display date and time (row 1)
                date_time = time_str.split(" ")
                display_text(display, date_time[0], X_COL1, 0, 1)  # Date
                display_text(display, date_time[1], X_COL1, ROW_HEIGHT, 1)  # Time
                
                # Display temperature (row 2)
                display_text(display, "Temp:", X_COL2, ROW_HEIGHT*2, 1)
                display_text(display, f"{temperature:.1f}C", X_COL2, ROW_HEIGHT*3, 1)
                
                # Display relative humidity (row 2, column 2)
                display_text(display, "RH:", X_COL3, ROW_HEIGHT*2, 1)
                display_text(display, f"{humidity:.1f}%", X_COL3, ROW_HEIGHT*3, 1)
                
                # Display absolute humidity (row 3)
                display_text(display, "AH:", X_COL4, ROW_HEIGHT*4, 1)
                display_text(display, f"{abs_humidity:.1f}g/m3", X_COL4, ROW_HEIGHT*5, 1)
                
                # Display pressure (row 3, column 2)
                display_text(display, "Press:", X_COL5, ROW_HEIGHT*4, 1)
                display_text(display, f"{pressure:.0f}hPa", X_COL5, ROW_HEIGHT*5, 1)
                
                # Display gas resistance (row 4)
                display_text(display, "Gas:", X_COL6, ROW_HEIGHT*6, 1)
                ...
                if gas > 1000000:
                    ...
                    gas_formatted = f"{gas}"
                display_text(display, f"{gas_formatted}ohm", X_COL6, ROW_HEIGHT*7, 1)
...
if __name__ == "__main__":
    ...
    print("Version: 1.1 - Static Display")
