...
The display shows uptime, CPU usage, memory usage, temperature, humidity, absolute humidity, 
pressure, and gas resistance.

All information is displayed in a row-based layout with improved formatting:
- Uptime is displayed in days, hours, minutes, seconds format
- CPU and memory usage are displayed as percentages
...
import math
import gc
from machine import I2C, SPI, Pin, WDT, reset, freq
import sys
import framebuf
import random
import os
...
def get_uptime():
    ...
    Returns:
        str: Formatted uptime string in the format "Xd Xh Xm Xs"
    """
    global start_time
...
def get_cpu_usage():
    """
    Get the current CPU frequency as a percentage of the maximum frequency.
    ...
    percentage = (current_freq / max_freq) * 100
...
def get_memory_usage():
    ...
    # Calculate percentage with decimal precision
    percentage = (mem_alloc / total_mem) * 100
    
    return percentage

# Function to get formatted timestamp for logs
def get_log_timestamp():
    """
    Get a formatted timestamp for log entries.
    
    Returns:
        str: Formatted timestamp with uptime
    """
    return f"Uptime: {get_uptime()}"
            ...
            if LOG_FILE in os.listdir():
                ...
                if os.stat(LOG_FILE)[6] > MAX_LOG_SIZE:
                    with open(LOG_FILE, "w") as f:
                        f.write(f"{get_log_timestamp()}: Log file truncated\n")
        except:
    ...
    try:
        ...
        while True:
            try:
                ...
                # Row 2: CPU and Memory usage (showing only percentage values with one decimal place)
                display_text(display, f"{cpu_usage:.1f}%/{memory_usage:.1f}%", X_OFFSET, ROW_START_Y + ROW_HEIGHT, 1)
                ...
                # Wait for next update (2 seconds to reduce system load)
                time.sleep(2.0)
...
if __name__ == "__main__":
    ...
    print("Version: 2.0 - Standalone with System Monitoring")
