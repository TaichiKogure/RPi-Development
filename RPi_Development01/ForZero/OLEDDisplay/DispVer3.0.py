...
Description: Displays date/time, CPU usage, and temperature on an OLED display
             with independently scrolling text and enhanced 3D animation.
"""
...
# スクロール位置の初期化
date_pos = WIDTH
usage_pos = -1000  # 画面外の左側
temp_pos = WIDTH

# スクロール速度の初期化
date_speed = random.uniform(scroll_speed_min, scroll_speed_max)
usage_speed = random.uniform(scroll_speed_min, scroll_speed_max)
temp_speed = random.uniform(scroll_speed_min, scroll_speed_max)

# スクロール遅延の初期化
date_delay = random.randint(scroll_delay_min, scroll_delay_max)
usage_delay = random.randint(scroll_delay_min, scroll_delay_max)
temp_delay = random.randint(scroll_delay_min, scroll_delay_max)
...
def main():
    """メイン処理ループ"""
    global cycle_count, date_pos, usage_pos, temp_pos
    global date_speed, usage_speed, temp_speed
    global date_delay, usage_delay, temp_delay
    ...
    try:
        while True:
            ...
            # スクロール速度をランダムに変更する可能性
            if random.random() < scroll_random_chance:
                date_speed = random.uniform(scroll_speed_min, scroll_speed_max)
            if random.random() < scroll_random_chance:
                usage_speed = random.uniform(scroll_speed_min, scroll_speed_max)
            if random.random() < scroll_random_chance:
            ...
            if date_delay > 0:
                ...
                date_pos -= date_speed
                if date_pos < -date_width:
                    date_pos = WIDTH
                    date_delay = random.randint(scroll_delay_min, scroll_delay_max)
                    date_speed = random.uniform(scroll_speed_min, scroll_speed_max)
            
            # テキストが画面内に入る範囲を計算
            if date_pos < WIDTH and date_pos > -date_width:
                # 新しい画像を作成してテキストを描画
                text_img = Image.new("1", (WIDTH, line_height), 0)
                text_draw = ImageDraw.Draw(text_img)
                date_y = (line_height - date_height) // 2  # 垂直方向中央揃え
                text_draw.text((date_pos, date_y), date_str, font=font_large, fill=255)
            ...
            if usage_delay > 0:
                ...
                usage_pos += usage_speed
                if usage_pos > WIDTH:
                    usage_pos = -usage_width
                    usage_delay = random.randint(scroll_delay_min, scroll_delay_max)
                    usage_speed = random.uniform(scroll_speed_min, scroll_speed_max)
            
            # テキストが画面内に入る範囲を計算
            if usage_pos < WIDTH and usage_pos > -usage_width:
                # 新しい画像を作成してテキストを描画
                text_img = Image.new("1", (WIDTH, line_height), 0)
                text_draw = ImageDraw.Draw(text_img)
                usage_y = (line_height - usage_height) // 2  # 垂直方向中央揃え
                text_draw.text((usage_pos, usage_y), cpu_usage, font=font_small, fill=255)
            ...
            if temp_delay > 0:
                ...
                temp_pos -= temp_speed
                if temp_pos < -temp_width:
                    temp_pos = WIDTH
                    temp_delay = random.randint(scroll_delay_min, scroll_delay_max)
                    temp_speed = random.uniform(scroll_speed_min, scroll_speed_max)
            
            # テキストが画面内に入る範囲を計算
            if temp_pos < WIDTH and temp_pos > -temp_width:
                # 新しい画像を作成してテキストを描画
                text_img = Image.new("1", (WIDTH, line_height), 0)
                text_draw = ImageDraw.Draw(text_img)
                temp_y = (line_height - temp_height) // 2  # 垂直方向中央揃え
                text_draw.text((temp_pos, temp_y), cpu_temp, font=font_small, fill=255)
