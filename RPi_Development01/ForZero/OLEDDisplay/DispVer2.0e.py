...
def main():
    ...
    try:
        while True:
            ...
            # スクロール処理
            # 最大スクロール幅を計算（最も長いテキストに合わせる）
            max_scroll = max(date_width, usage_width, temp_width) + space + WIDTH
            
            try:
                # 右から左へのスクロール用のステップ数
                steps_rl = max_scroll // scroll_speed_rl
                # 左から右へのスクロール用のステップ数
                steps_lr = max_scroll // scroll_speed_lr
                ...
                for step in range(max_steps):
                    ...
                    # 1. 上段（日時）- 右から左へスクロール
                    # 右端から徐々に現れ、左端へ消えていく
                    pos_rl = step * scroll_speed_rl
                    if pos_rl < max_scroll:
                        # 右から左へのスクロール位置を計算
                        start_pos = max(0, date_total_width - WIDTH - pos_rl)
                        date_crop = date_img.crop((start_pos, 0, start_pos + WIDTH, line_height))
                        display_img.paste(date_crop, (0, 0))
                    
                    # 2. 中段（CPU使用率）- 左から右へスクロール
                    # 左端から徐々に現れ、右端へ消えていく
                    pos_lr = step * scroll_speed_lr
                    if pos_lr < max_scroll:
                        # 左から右へのスクロール位置を計算
                        start_pos = max(0, pos_lr - usage_width)
                        usage_crop = usage_img.crop((start_pos, 0, start_pos + WIDTH, line_height))
                        display_img.paste(usage_crop, (0, line_height))
                    ...
                    if pos_rl < max_scroll:
                        # 右から左へのスクロール位置を計算
                        start_pos = max(0, temp_total_width - WIDTH - pos_rl)
                        temp_crop = temp_img.crop((start_pos, 0, start_pos + WIDTH, line_height))
                        display_img.paste(temp_crop, (0, line_height * 2))
