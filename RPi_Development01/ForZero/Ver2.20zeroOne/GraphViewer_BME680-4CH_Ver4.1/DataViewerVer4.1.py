...
変更点（Ver4.1 パッチ）:
- 表示期間の「Display range（days）」機能を削除しました。
- 代わりに「直近24時間」のデータのみを表示します（固定）。
- グラフは10分ごと（600秒）に自動更新されます。
- Min/Max の概要表は表示中（直近24時間）の範囲で計算されます。

データソース:
- P1システムが生成する各デバイス用の固定CSV（P1..P4、CO2なし）
...
API:
- GET /api/graphs?show_p1=true&show_p2=true&show_p3=true&show_p4=true
  温度・湿度・絶対湿度・気圧・ガス抵抗に加え、不快指数および簡易汚染指標を返します。
  返却データは常に「直近24時間」に限定されます（クエリのdaysは無視）。

- GET /download/series?device=P2&param=discomfort_index
  選択したデバイス・パラメータの時系列CSV（直近24時間）を返します（daysは無視）。

- GET /download/p1 .. /download/p4
  各デバイスの固定CSVファイル自体（全データ）を返します。
...
- sudo ~/envmonitor-venv/bin/python3 start_data_viewer_ver4.1.py
  ブラウザで http://<P1のIP>:8081 を開くと、直近24時間のグラフが表示され、10分ごとに自動更新されます。
"""
...
<body>
  ...
  <div class=\"downloads\">
    <strong>CSVダウンロード（直近24時間・選択デバイス）:</strong>
  ...
  <script>
    ...
    function buildDownloadLinks(){
      ...
      paramMeta.forEach(p => {
        devices.forEach(d => {
          ...
          a.href = `/download/series?device=${d}&param=${p.id}`; // 直近24時間（固定）
    ...
    function renderStatsTable(stats){
      ...
      html += '</tbody></table>';
      html += '<caption>直近24時間・選択デバイスに基づき算出。</caption>';
