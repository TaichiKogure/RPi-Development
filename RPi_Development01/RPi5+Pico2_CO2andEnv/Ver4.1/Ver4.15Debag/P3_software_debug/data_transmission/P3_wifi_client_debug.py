# -*- coding: utf-8 -*-
"""
WiFi Client for Raspberry Pi Pico 2W (P3) - Debug Version 4.15
...
- Detailed status reporting at each connection step
- Configurable connection parameters
- Multiple connection strategies
- Disable/enable automatic reset
...
Usage:
    import P3_wifi_client_debug
    client = P3_wifi_client_debug.WiFiClient(ssid="RaspberryPi5_AP_Solo", password="raspberry")
    client.connect(debug_level=2, auto_reset=False)
...
class WiFiClient:
    ...
    def __init__(self, ssid="RaspberryPi5_AP_Solo", password="raspberry", 
        ...
        # Print configuration
        self._debug_print(f"WiFi Client initialized for {device_id} (Debug Version 4.18)", DEBUG_BASIC)
        ...
        while not self.wlan.isconnected() and retry_count < max_retries:
            try:
                ...
                while not self.wlan.isconnected() and timeout < connection_timeout:
                    ...
                    # Print progress and status more frequently in debug mode
                    if timeout % (5 if self.debug_level <= DEBUG_BASIC else 2) == 0:
                        ...
                        # Print more detailed status in verbose mode
    ...
    def _print_connection_info(self):
        ...
        try:
            ...
            self._debug_print(f"Subnet mask: {network_info[1]}", DEBUG_BASIC)
            self._debug_print(f"Gateway: {network_info[2]}", DEBUG_BASIC)
            self._debug_print(f"DNS: {network_info[3]}", DEBUG_BASIC)

            # Get signal strength
            rssi = self.wlan.status('rssi')
            self._debug_print(f"Signal strength: {rssi} dBm", DEBUG_BASIC)
    ...
    def run(self, run_once=False):
        ...
        if not self.wifi_client.is_connected():
            ...
            if not self.wifi_client.connect(connection_timeout=45):
                self._debug_print("Failed to connect to WiFi. Cannot transmit data.", DEBUG_BASIC)
                return False
        ...
        if run_once:
            ...
            # Run continuously
            try:
                self._debug_print("Starting continuous data transmission...", DEBUG_BASIC)
                while True:
                    self.collect_and_send_data()
                    time.sleep(1)  # Small delay to prevent CPU overload
            except KeyboardInterrupt:
                self._debug_print("Data transmission stopped by user", DEBUG_BASIC)
                return True
            except Exception as e:
                self._debug_print(f"Error in data transmission: {e}", DEBUG_BASIC)
                return False
...
if __name__ == "__main__":
    try:
        ...
        print("=== P3 WiFi Client Debug Test (Ver 4.15.0) ===")
        ...
        client = WiFiClient(device_id="P3", debug_level=DEBUG_DETAILED)
