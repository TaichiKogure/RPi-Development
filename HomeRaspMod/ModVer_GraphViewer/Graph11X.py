...
Graph11X
---------
Hardened CSV reader/visualizer with robust normalization pipeline.
Highlights vs Graph10X:
- Stronger column normalization (trim/lower/delimiter unify + alias map), collision-safe, logs converted/skipped.
- Tolerant datetime parsing (full-width → half-width, '/'→'-', allow 'Z' TZ, try epoch s/ms and multiple formats).
- Numeric cleansing with unit/annotation stripping (°C, %, hPa, ppm, Ω/kΩ→×1000, commas, null tokens), per-file stats.
- Expanded alias sets and suffix variants (_outside, _ds18b20, _inside).
- Absolute humidity auto-derivation from prioritized temp/RH column pairs.
- Duplicate header and blank row removal strengthened.
- Incremental read cache; normalization reapplied after merges.
- Resampling rules use lower-case ("s", "min", "h").
- Detailed logging with drop-rate threshold emphasis.

Usage:
  python Graph11X.py

Edit DATA_FILES/REQUIRED_COLS to match your environment if needed.
"""

# =========================
# Logger setup
# =========================
logger = logging.getLogger("Graph11X")
...
SUFFIX_VARIANTS = ["_outside", "_ds18b20", "_inside", "_out"]
...
def normalize_columns_collision_safe(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, str], List[str], Dict[str, int]]:
    """
    ...
    Returns: (df_renamed, rename_map_used, skipped_original_columns, stats)
    stats: {'converted': n, 'skipped': n, 'collisions': n, 'unmapped_passthrough': n}
    """
    ...
    stats = {
        ...
        'unmapped_passthrough': unmapped_passthrough,
    }
    return df2, rename_map, skipped, stats

# =========================
# Datetime tolerant parsing
# =========================

def _preprocess_datetime_text(x: str) -> str:
    # Replace trailing 'Z' with '+00:00' to make %z parseable
    s = x.strip()
...
def preprocess_datetime_series(s: pd.Series) -> pd.Series:
    # to string, strip, full-width -> half-width
    s2 = s.astype(str).str.strip()
    s2 = s2.str.translate(_fullwidth_map)
    s2 = s2.apply(_preprocess_datetime_text)
    return s2
...
UNIT_PATTERNS = [
    ...
    (re.compile(r'\s*Ω'), ''),
    (re.compile(r'\s*g/m3', re.IGNORECASE), ''),
    (re.compile(r'\s*g/m²', re.IGNORECASE), ''),
]
...
def _normalize_pipeline(df: pd.DataFrame, date_column_original: str, file_path: str) -> pd.DataFrame:
    ...
    df, rename_map, skipped, col_stats = normalize_columns_collision_safe(df)
    ...
    if total_unit_hits or total_k_hits:
        logger.info(f"{os.path.basename(file_path)}: unit removals={total_unit_hits}, k-mults={total_k_hits}, numeric_cols_cleaned={converted_cols}")
    ...
    # 6) summarize column normalization
    logger.info(
        f"{os.path.basename(file_path)}: columns converted={col_stats['converted']}, "
        f"unmapped_passthrough={col_stats['unmapped_passthrough']}, collisions={col_stats['collisions']}, "
        f"skipped={col_stats['skipped']}"
    )
...
def _read_csv_incremental(file_path: str, date_column: str, usecols: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    # Apply normalization pipeline (covers header duplicates, datetime, numeric cleansing, range checks)
...
# =========================
# Smoke tests (minimal regression tests)
# =========================

def run_smoke_tests():
    """Minimal tests for normalization, datetime parsing, numeric cleansing."""
    logger.info("Running Graph11X smoke tests...")
    df = pd.DataFrame({
        ' Current Time ': ['2025/08/24 12:00:00', '2025-08-24T12:00:30Z', '  1724491260  ', 'bad'],
        ' Tempereture ': ['30.2°C', '29.8 C', '30,100', '31.0'],
        ' Humidity-outside': ['45 %', ' 110 % ', 'NaN', '--'],
        'GasResistance': ['10 kΩ', '9500 ohm', ' 12,300 ', 'NA'],
        'CO₂': ['420 ppm', ' 800 ', '—', 'null'],
    })
    df_norm = _normalize_pipeline(df, 'current_time', 'SMOKE.csv')

    assert 'temperature' in df_norm.columns, 'temperature should be normalized from Tempereture'
    assert 'humidity_outside' in df_norm.columns, 'humidity_outside alias expected'
    assert 'gas_resistance' in df_norm.columns, 'gas_resistance normalized'
    assert 'co2' in df_norm.columns, 'co2 normalized from CO₂'
    assert isinstance(df_norm.index, pd.DatetimeIndex), 'Datetime index expected'
    # Ensure numeric types
    for col in ['temperature', 'humidity_outside', 'gas_resistance', 'co2']:
        if col in df_norm.columns:
            assert pd.api.types.is_float_dtype(df_norm[col]), f'{col} should be float dtype'
...
if __name__ == '__main__':
    # Optional: run tests before plotting
    try:
        run_smoke_tests()
    except Exception as e:
        logger.warning(f"Smoke tests failed: {e}")
