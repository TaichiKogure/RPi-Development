...
def run_all_cycles(num_cycles=50, c_rate=0.5, Q0=3.0, A=0.1, B=0.05, R0=0.05, C=0.05, D=1.0, 
    ...
    for n in range(1, num_cycles+1):
        ...
        results.append((n, cap_n, res_n, soc_c, cap_c, v_c, soc_d, cap_d, v_d))
    return results
...
def plot_all_smoothed(results, smoothing_s=0.001):
    ...
    # Charge side
    plt.subplot(1,2,0+1)
    for n, _, _, _, cap_c, v_c, _, _, _ in results:
    ...
    # Discharge side
    plt.subplot(1,2,2)
    for n, _, _, _, _, _, _, cap_d, v_d in results:
...
def export_to_csv(results, filename_prefix):
    ...
    with open(summary_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Cycle', 'Capacity (Ah)', 'Resistance (ohm)'])
        for n, cap_n, res_n, _, _, _, _, _, _ in results:
            writer.writerow([n, cap_n, res_n])
    print(f"サイクル概要を保存しました: {summary_filename}")
    
    # Export detailed charge/discharge data for each cycle
    for n, _, _, soc_c, cap_c, v_c, soc_d, cap_d, v_d in results:
        # Charge data
...
def interactive_mode():
    ...
    print("=" * 60)
    print("リチウムイオン電池の劣化シミュレーション - 対話モード")
    print("=" * 60)
    ...
    # Plot results
    plot_all_smoothed(results, smoothing_s=smoothing_s)
