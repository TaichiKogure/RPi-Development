...
# -*- coding: utf-8 -*-
"""
Raspberry Pi 5 Environmental Data Collection Module for Solo Version 4.44
Version: 4.44.0-solo
...
Usage:
    python3 P1_data_collector_solo44.py [--port PORT] [--data-dir DIR]
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/var/log/data_collector_solo44.log"),
...
try:
    from P1_wifi_monitor_solo44 import WiFiMonitor, DEFAULT_CONFIG as MONITOR_CONFIG
            ...
            if data:
                try:
                    ...
                    if self._validate_data(json_data):
                        ...
                        client_socket.sendall(b'{"status": "error", "message": "無効なデータ形式です"}')
                except json.JSONDecodeError as e:
                    logger.error(f"Failed to parse JSON data: {e}")
                    client_socket.sendall(b'{"status": "error", "message": "無効なJSON形式です"}')
            else:
                logger.warning("Received empty data")
                client_socket.sendall(b'{"status": "error", "message": "データが空です"}')

        except socket.timeout:
            logger.warning(f"Connection timeout from {addr}")
            client_socket.sendall(b'{"status": "error", "message": "接続がタイムアウトしました"}')
        except Exception as e:
            logger.error(f"Error handling client {addr}: {e}")
            try:
                client_socket.sendall(b'{"status": "error", "message": "サーバーエラーが発生しました"}')
            except:
                pass
        finally:
            client_socket.close()
