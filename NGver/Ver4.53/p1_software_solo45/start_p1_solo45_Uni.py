...
def main():
    """すべてのサービスを開始するメイン関数。"""
    parser = argparse.ArgumentParser(description="Raspberry Pi 5 Unified Startup Script for Solo Version 4.53 (Unified Version)")
    ...
    parser.add_argument("--p2-path", type=str, default=DEFAULT_CONFIG["p2_csv_path"],
                        help=f"P2のCSVデータファイルのパス (デフォルト: {DEFAULT_CONFIG['p2_csv_path']})")
    parser.add_argument("--p3-path", type=str, default=DEFAULT_CONFIG["p3_csv_path"],
                        help=f"P3のCSVデータファイルのパス (デフォルト: {DEFAULT_CONFIG['p3_csv_path']})")
    ...
    config["p2_csv_path"] = args.p2_path
    config["p3_csv_path"] = args.p3_path

    # data_dirが変更された場合、p2_csv_pathとp3_csv_pathも更新（引数で明示的に指定されていない場合）
    if args.data_dir != DEFAULT_CONFIG["data_dir"] and args.p2_path == DEFAULT_CONFIG["p2_csv_path"]:
        config["p2_csv_path"] = os.path.join(args.data_dir, DEFAULT_CONFIG["rawdata_p2_dir"], "P2_fixed.csv")
    if args.data_dir != DEFAULT_CONFIG["data_dir"] and args.p3_path == DEFAULT_CONFIG["p3_csv_path"]:
        config["p3_csv_path"] = os.path.join(args.data_dir, DEFAULT_CONFIG["rawdata_p3_dir"], "P3_fixed.csv")
